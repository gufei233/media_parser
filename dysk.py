...
class ABogus:
    ...
    def write(self, e):
        self.size = len(e)
        if isinstance(e, str):
            e = self.char_code_at(self.decode_string(e))
        if len(e) <= 64:
            self.chunk = e
        else:
    ...
    def extract_data(self, data_dict: dict):
        result = {
            ...
            "author": {
                ...
                "avatar": self.safe_extract(
                    data_dict, "author.avatar_thumb.url_list[0]"
                ),
            },
            "statistics": {
                "digg_count": self.safe_extract(data_dict, "statistics.digg_count"),
                "comment_count": self.safe_extract(
                    data_dict, "statistics.comment_count"
                ),
                "collect_count": self.safe_extract(
                    data_dict, "statistics.collect_count"
                ),
        ...
        if images:
            ...
            if has_live:
                ...
                result["type"] = "图集"
                result["downloads"] = [
                    self.safe_extract(i, "url_list[0]") for i in images
                ]
        else:
            result["type"] = "视频"
            ...
            result["downloads"] = [
                {"type": "video", "url": video_url, "cover": cover_url}
            ]
